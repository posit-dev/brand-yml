...
Each theme color can be a single color or a light/dark mapping.
...
Use a `light`/`dark` mapping when a semantic color needs to change with the active color scheme.
For example, foreground and background colors usually need opposite values in dark mode.
...
Light and dark values are optional, so a mapping may override only one mode.
An undefined mode keeps the value provided by the consuming theme. A scalar
color, such as `primary` above, applies in both modes.

`color.palette` remains a mapping of scalar color values. Theme colors may
reference palette colors or other theme colors. A reference inside a `light`
or `dark` mapping resolves in that same mode. If the referenced color does not
define that mode, the reference is undefined and the consuming theme keeps its
base value.
...
Palette references can also be used inside `light` and `dark` mappings for
theme colors.
