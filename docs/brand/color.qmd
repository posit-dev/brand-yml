...
The available value is used when the requested mode is missing. A scalar color,
such as `primary` above, works in both modes.
...
or `dark` mapping resolves in that same mode; if it falls back to the other
mode, the referenced color is resolved in the fallback mode as well.
