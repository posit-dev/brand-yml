...
### Light/Dark Typography Colors

Typography `color` and `background-color` values can use light/dark mappings,
just like theme colors. They can also reference a theme color that has
light/dark variants.
...
A scalar value works in both modes. A mapping may provide only `light` or
`dark`; the missing mode keeps the value provided by the consuming theme.
References inside a variant resolve only in that variant's color mode.
