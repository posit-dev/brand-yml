...
You cn specify different logos for light and dark backgrounds by using giving the `small`, `medium`, and `large` attributes an object with `light` and `dark` properties.
