...
Because these names are provided by users, it might be better to use a prefix like `$palette-` for the Sass variables.
(If it were easier to extract items from a Sass map, we'd use that instead, but `map-get($palette, orange)` isn't user-friendly.)
